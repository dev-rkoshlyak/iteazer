...
Driver.prototype.streamCollisions = function(sps, remove) {
  this.sphero.detectCollisions({device: "bb8"});
  var self = this;
  this.sphero.on("collision", function(data) {
    console.log("Collision!");
    console.log(JSON.stringify(data));
    self.emit("Collisions", data);
  });
};
...
Driver.prototype.stopOdometer = function() {
    var device = this.sphero;
    device.streamData({
      event: "odometer",
      mask2: 0x0C000000,
...
Driver.prototype.stopVelocity = function() {
  var device = this.sphero;
  device.streamData({
    event: "velocity",
    mask2: 0x01800000,
...
Driver.prototype.stopAccelOne = function() {
    var device = this.sphero;
    device.streamData({
      event: "accelOne",
      mask2: 0x02000000,
...
Driver.prototype.stopImuAngles = function() {
    var device = this.sphero;
    device.streamData({
      event: "imuAngles",
      mask1: 0x00070000,
...
Driver.prototype.stopAccelerometer = function() {
    var device = this.sphero;
    device.streamData({
      event: "accelerometer",
      mask1: 0x0000E000,
...
Driver.prototype.stopGyroscope = function() {
    var device = this.sphero;
    device.streamData({
      event: "gyroscope",
      mask1: 0x00001C00,
...
Driver.prototype.stopMotorsBackEmf = function() {
    var device = this.sphero;
    device.streamData({
      event: "motorsBackEmf",
      mask1: 0x00000060,
...
Driver.prototype.setRawMotors = function(leftCommand, leftPower,
                                         rightCommand, rightPower, callback) {
  var opts = {
  ...
  this.sphero.setRawMotors(opts, callback);
};
...
Driver.prototype.setStabilization = function(stabiliaztion, callback) {
  this.sphero.setStabilization(stabiliaztion, callback);
};
